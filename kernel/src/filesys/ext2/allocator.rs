...
use super::structures::{Superblock, BlockGroupDescriptor};
use super::cache::Cache;
use super::cache::block::CachedBlock;
...
impl Allocator {
    ...
    fn allocate_block_in_group(&self, group: usize) -> AllocResult<u32> {
        ...
        let _guard = self.group_locks.get(group)
            .ok_or(AllocError::InvalidNumber)?
            .lock();

        // Read the bitmap
        let bitmap_block = self.block_cache.get(bgdt.block_bitmap_block)
            .map_err(|_| AllocError::CacheError)?;
        let mut bitmap_block = bitmap_block.lock();
        let bitmap = bitmap_block.data_mut();

        // Find a free block
        let bit = Self::find_first_zero(bitmap)
    ...
    fn allocate_inode_in_group(&self, group: usize) -> AllocResult<u32> {
        ...
        // Read the bitmap
        let bitmap_block = self.block_cache.get(bgdt.inode_bitmap_block)
            .map_err(|_| AllocError::CacheError)?;
        let mut bitmap_block = bitmap_block.lock();
        let bitmap = bitmap_block.data_mut();
